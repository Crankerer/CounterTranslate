...
- `app/updater.py` — Checks GitHub releases (repo `Crankerer/CounterTranslate`) on startup when running as frozen EXE (`maybe_update(prereleases=False)`). Only stable (non-draft, non-prerelease) releases are considered; if the release listing contains no stable release, no update happens. If a newer version is found, shows a yes/no dialog asking the user to confirm before downloading. On confirmation, shows a `_UpdateUI` progress window while downloading. Stages the update in `update_pending/`, then restarts via the launcher which applies it.
...
**Important:** the auto-updater (`maybe_update(prereleases=False)`) ignores prereleases, so a CI-created release is invisible to existing installs until you manually edit it on GitHub and uncheck "pre-release". This is the intended promotion step after verifying the build.
