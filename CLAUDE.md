...
- `app/config.py` — `load_config` / `save_config` around `config.json`. Merges `DEFAULTS` for backward compatibility. `save_config` is **atomic**: it writes a `.config-*.tmp` in the same directory, `fsync`s, then `os.replace`s it into place under `_SAVE_LOCK`, so the tailer's 5-second reload can never read a half-written file (on Windows `os.replace` is retried briefly — it fails while a reader holds the destination open). `load_config` retries a parse failure a few times (absorbing any torn read), and a file that stays unreadable is moved to `config.json.bak` and logged as an ERROR rather than silently overwritten with defaults. Exports `DEFAULT_API_URL` (the built-in proxy endpoint used when `gpt_api` is empty). Supports an `open_ai_api_key_file` pointer as an alternative to embedding the key (config only, not exposed in settings UI). `DEFAULTS` contains legacy keys `llm_api` and `llm_model` (kept for backward compatibility with older config files; not used at runtime).
...
- `app/http_session.py` — Module-level `SESSION`: a shared `requests.Session` with retry/backoff (2 retries, `backoff_factor=0.4`, retries on 429/5xx). Deliberately short: this serves live chat translation, where a late answer is useless. Combined with `call_chatgpt`'s 10 s per-attempt timeout the worst case is ~31 s per message. Import and reuse this instead of creating new sessions.
...
| `"status"` | `color, tooltip` | Proxy status change, applied via `_apply_status`; how `set_status_color` reaches the Tk thread |
...
Three more rules, each of which had a bug behind it:

- **Never call Tkinter from a non-Tk thread** — not even `root.after`. Push a message onto the HUD queue and handle it in `_poll` (see `set_status_color`). Methods that are Tk-thread-only say so in their docstring.
- **`_HudStream.write` holds a lock.** `print()` emits several `write()` calls per line and is called from the tail thread, all pool workers and the Tk thread; without the lock, lines from different threads splice together in both the HUD and the log file.
- **`config.json` has one writer thread (Tk) and one reader thread (tailer).** Writes must stay atomic — use `save_config`, never `open(path, "w")` directly.
...
| `DEBUG` | `[chat]` every detected CS2 message, `[ignored]` messages from ignored players, `[llm_request]` outgoing API call, `[llm_response]` translated result, `[skipped_lang]` messages suppressed after LLM detected a skip language, `[skipped_empty]` LLM returned empty, `[skipped_stale]` message waited longer than `_MAX_QUEUE_AGE_S` in the pool queue |
